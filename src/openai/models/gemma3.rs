...
impl RotaryEmbedding {
    ...
    fn new(dtype: DType, cfg: &Config, dev: &Device) -> Result<Self> {
        ...
        let (sin_sliding, cos_sliding, cos_sin_sliding) = if cfg.sliding_window.is_some() {
            let (sin, cos) = RotaryEmbedding::create_cache(dtype, cfg.sliding_window, cfg, dev)?;
            let cos_sin_sliding = Tensor::cat(&[&cos, &sin], candle::D::Minus1)?.contiguous()?; //must be contiguous tensor;
            (Some(sin), Some(cos), Some(cos_sin_sliding))
        } else {
            (None, None, None)
        };
    ...
    fn apply_rotary_emb_qkv(
        ...
        if q.device().is_gcu() {
            ...
            let cos_sin = if is_sliding && self.sin_sliding.is_some() && self.cos_sliding.is_some()
            {
            ...
            for (b, seqlen_offset) in zip(0..b_sz, input_positions) {
                let (cos, sin) =
                    if is_sliding && self.sin_sliding.is_some() && self.cos_sliding.is_some() {
                        let cos = self.cos_sliding.as_ref().unwrap().narrow(
                            0,
                            seqlen_offset[0],
                            seq_len,
                        )?;
                        let sin = self.sin_sliding.as_ref().unwrap().narrow(
                            0,
                            seqlen_offset[0],
                            seq_len,
                        )?;
